...
public class GezagsmoduleInterfaceApiDelegateServiceV2 implements GezagsmoduleInterfaceApiDelegate {
    ...
    public ResponseEntity<GezagResponse> opvragenBevoegdheidTotGezag(
        ...
        var gezagRequestV1 = new nl.rijksoverheid.mev.web.api.v1.GezagRequest().burgerservicenummer(burgerservicenummers);

        var personenDeprecated = bevoegdheidTotGezagService.bepaalBevoegdheidTotGezag(gezagRequestV1);
        var personen = personenDeprecated.stream()
            .map(Personen::from)
            .toList();

        GezagResponse response = new GezagResponse();
        response.setPersonen(personen);
